...
** PHP
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package php-mode :defer t :ensure t :mode "\\.php$"
       :init (add-hook 'php-mode-hook (lambda() (add-to-list (make-local-variable 'company-backends)
...
* Useful Packages
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (use-package ace-window :defer t :commands (ace-window) :init (global-set-key (kbd "M-i") 'ace-window))
